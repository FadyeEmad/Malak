...
let photoUrls = (window.settings?.heartPhotos?.length > 0)
    ? window.settings.heartPhotos
    : pages.filter(page => page.image).map(page => page.image);
...
let maxHeartPhotos = 0; // سيتم حسابها بشكل ديناميكي - كل صورة تظهر 4 مرات
...
function spawnHeartPhotosCentered() {
    ...
    // حساب عدد الصور: كل صورة تظهر 4 مرات
    maxHeartPhotos = photoUrls.length * 4;

    // Preload تất cả ảnh trước
